...
public class AddUserPermission : IGroupCommand
{
    ...
    public async Task OnPermissionGrantedAsync(string type, GroupMessageEventArgs eventArgs)
    {
        ...
        if (args.Length != 3)
        {
            await eventArgs.SourceGroup.SendGroupMessage("您输入的参数不符合要求。请参考以下语法进行输入：添加用户权限 <角色昵称/QQ/at> <权限名>");
            return;
        }
        ...
        {
            await eventArgs.SourceGroup.SendGroupMessage("该用户已经有此权限。");
            return;
        }
        
        player.AddPermission(args[2]);
        await db.SaveChangesAsync();
        await eventArgs.SourceGroup.SendGroupMessage("已成功添加权限。");
    }
}