...
public static class CommandManager
{
    ...
    private static async ValueTask OnGroupMessage(string eventType, GroupMessageEventArgs args)
    {
        ...
        foreach (var command in RegisteredGroupCommands.Where(x => args.Message.GetCommandArgs()[0] == x.GetCommand() || x.GetCommand() == string.Empty))
        {
            if (args.IsSuperUser)
            {
                await CatchAndLog(() =>
    ...
    private static async ValueTask OnPrivateMessage(string eventType, PrivateMessageEventArgs args)
    {
        ...
        foreach (var command in RegisteredPrivateCommands.Where(x => args.Message.GetCommandArgs()[0] == x.GetCommand() || x.GetCommand() == string.Empty))
        {
            if (args.IsSuperUser)
            {
                await CatchAndLog(() =>
